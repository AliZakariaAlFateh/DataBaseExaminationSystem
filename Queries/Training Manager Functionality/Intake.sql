...
create or alter procedure TrainingManagerFunctionality.addIntake @intake_round int,@intake_name varchar(50),
			@track_code varchar(5), @dept_name nvarchar(50),@branch_name nvarchar(50)
as
	begin try
		begin transaction
			if not exists(select * from Intake where intake_round = @intake_round)
			begin
				...
				print 'There is an Intake with that round'
		commit
	end try
	begin catch
		rollback
...
as
	begin try
		...
		begin
			if exists(select * from Branch where branch_name = @branch_name)
			begin
				if exists(select * from Br_Dept_Tr_Intake where intake_round = @intake_round and branch_name = @branch_name)
					print concat('The Round num "',@intake_round,'" is already exists on the "',@branch_name,'" branch')
				else
				begin
					declare @dept_name nvarchar(50) ,@track_code varchar(5)
				...
				print concat('The Branch with name "',@branch_name,'" is not exists')
		end
		else
			print concat('The Round num "',@intake_round,'" is not exists')
	end try
	begin catch
		rollback
		print ERROR_MESSAGE()
